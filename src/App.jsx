...
function CaseForm({init,onSave,onCancel,opts}){
  ...
  return <div>
    ...
    <div style={{background:"#FFF8F0",borderRadius:12,padding:"16px 16px 4px",marginBottom:16,border:"1px solid #FFE0B2"}}>
      ...
      <Lb label="メモ"><textarea style={{...is,minHeight:140,resize:"vertical",lineHeight:1.6}} value={f.nextActionMemo||""} onChange={e=>s("nextActionMemo",e.target.value)} placeholder="補足"/></Lb>
...
function CaseMod(){
  ...
  const sortFn=(a,b)=>{if(sortBy==="manual"){if(a.order!=null&&b.order!=null)return a.order-b.order;if(a.order!=null)return-1;if(b.order!=null)return 1}const gk=c=>sortBy==="shootDate"?(c.shootDate||c.publishDate||c.deadline||"9999-12-31"):(c.publishDate||c.shootDate||c.deadline||"9999-12-31");return gk(a).localeCompare(gk(b))};
...
function SemMod(){
  ...
  const fil=sems.filter(c=>{if(isDone(c.status))return false;if(fs!=="すべて"&&c.status!==fs)return false;if(q)return(c.name+c.client+c.venue+c.memo+(c.nextAction||"")).toLowerCase().includes(q.toLowerCase());return true}).sort((a,b)=>{const ad=a.eventDate||"9999-12-31",bd=b.eventDate||"9999-12-31";if(ad!==bd)return ad.localeCompare(bd);return(a.name||"").localeCompare(b.name||"")});
...
const nextRecur=(day,from)=>{const n=from?new Date(from):new Date();n.setHours(0,0,0,0);const y=n.getFullYear(),m=n.getMonth();const lastDay=new Date(y,m+1,0).getDate();const d=Math.min(day,lastDay);let next=new Date(y,m,d);if(next<=n){const lm=new Date(y,m+2,0).getDate();next=new Date(y,m+1,Math.min(day,lm))}return next.getFullYear()+"-"+String(next.getMonth()+1).padStart(2,"0")+"-"+String(next.getDate()).padStart(2,"0")};

function TaskMod(){
  const[tasks,sT,ok]=useSt(TK);
  const[cases,sC]=useSt(CK);
  const[sems,sS]=useSt(SK);
  const[newT,sNewT]=useState("");const[newD,sNewD]=useState("");const[newCat,sNewCat]=useState("プライベート");const[newR,sNewR]=useState("");const[exp,sExp]=useState({});const[showDone,sShowDone]=useState(false);const[showArc,sShowArc]=useState(false);const[filter,sFilter]=useState("all");const[view,sView]=useState("board");const[boardMode,sBoardMode]=useState("category");const[dragId,sDragId]=useState(null);const[dragOver,sDragOver]=useState(null);const[qc,sQc]=useState({col:null,val:"",deadline:""});
  const add=()=>{if(!newT.trim())return;const r=parseInt(newR);const rd=r>=1&&r<=31?r:null;const dl=rd&&!newD?nextRecur(rd):newD;sT(p=>[...p,{id:uid(),title:newT.trim(),deadline:dl,memo:"",done:false,column:0,category:newCat,recurDay:rd,createdAt:new Date().toISOString(),order:p.length}]);sNewT("");sNewD("");sNewR("")};
  const completeOne=t=>{if(t.recurDay){const newDl=nextRecur(t.recurDay,t.deadline);sT(p=>p.map(x=>x.id===t.id?{...x,deadline:newDl,done:false,archived:false,column:0,lastCompletedAt:new Date().toISOString()}:x))}else{sT(p=>p.map(x=>x.id===t.id?{...x,done:true,archived:true,column:3,completedAt:new Date().toISOString(),archivedAt:new Date().toISOString()}:x))}};
  const toggle=id=>{if(id.startsWith("case:")||id.startsWith("sem:"))return toggleVirtual(id);const t=tasks.find(x=>x.id===id);if(!t)return;if(t.done){sT(p=>p.map(x=>x.id===id?{...x,done:false,archived:false,column:0,completedAt:null,archivedAt:null}:x))}else{completeOne(t)}};
  ...
  const dropOnCard=(targetId,colKey,siblings,e)=>{e.preventDefault();e.stopPropagation();const dId=dragId;sDragOver(null);sDragId(null);if(!dId||dId===targetId||dId.startsWith("case:")||dId.startsWith("sem:"))return;const rect=e.currentTarget.getBoundingClientRect();const before=e.clientY<rect.top+rect.height/2;const ids=siblings.map(t=>t.id).filter(i=>i!==dId);const ti=ids.indexOf(targetId);if(ti<0)return;ids.splice(before?ti:ti+1,0,dId);const posMap={};ids.forEach((tid,i)=>posMap[tid]=i);sT(p=>p.map(t=>{if(t.id===dId){const u={...t,manualPos:posMap[t.id]};if(boardMode==="category")u.category=colKey;else{u.column=colKey;u.done=colKey===3;if(colKey===3&&!t.completedAt)u.completedAt=new Date().toISOString()}return u}return posMap[t.id]!=null?{...t,manualPos:posMap[t.id]}:t}))};
    ...
    <div style={{maxWidth:view==="board"?"none":720,margin:"0 auto",padding:"20px 16px 40px"}}>
      ...
      {view==="list"&&<><div style={{background:"#fff",borderRadius:14,padding:"12px 14px",marginBottom:16,border:"1px solid #E3F2FD",boxShadow:"0 2px 8px rgba(25,118,210,.06)"}}>
        <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
          ...
          <input style={{...is,width:120,border:"1.5px solid #BBDEFB"}} type="number" min="1" max="31" value={newR} onChange={e=>sNewR(e.target.value)} placeholder="🔄 毎月X日"/>
      ...
      {filtered.length===0?<div style={{textAlign:"center",padding:40,color:"#B0BEC5"}}>{active.length===0?"タスクはありません。追加してみましょう！":"該当するタスクがありません"}</div>:<div>
        {filtered.map(t=>{const dy=dt(t.deadline);const urg=dy!==null&&dy<=0;const warn=dy!==null&&dy>0&&dy<=3;const open=exp[t.id];const isV=!!t.source;return <div key={t.id} style={{background:"#fff",borderRadius:12,marginBottom:8,border:urg?"1.5px solid #EF9A9A":warn?"1.5px solid #FFCC80":"1px solid #ECEFF1",overflow:"hidden",transition:"all .15s"}}>
          <div style={{display:"flex",alignItems:"center",gap:10,padding:"12px 14px",cursor:"pointer"}} onClick={()=>sExp(e=>({...e,[t.id]:!e[t.id]}))}>
            ...
            <div style={{flex:1,minWidth:0}}>
              <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>{isV?<span style={{fontSize:9,fontWeight:700,padding:"2px 7px",borderRadius:8,background:t.sourceColor,color:"#fff"}}>{t.source}</span>:(()=>{const c=CCOLS.find(x=>x.k===getCat(t));return c?<span style={{fontSize:9,fontWeight:700,padding:"2px 7px",borderRadius:8,background:c.hd,color:c.t}}>{c.l}</span>:null})()}{t.recurDay&&<span style={{fontSize:9,fontWeight:700,padding:"2px 7px",borderRadius:8,background:"#E1F5FE",color:"#01579B"}}>🔄 毎月{t.recurDay}日</span>}<div style={{fontSize:14,fontWeight:600,color:"#1A2A3A"}}>{t.title}</div></div>
          ...
          {open&&<div style={{padding:"0 14px 14px 46px",borderTop:"1px solid #F5F5F5"}}>
            ...
            <div style={{display:"flex",gap:8,marginBottom:8,flexWrap:"wrap"}}><input style={{...is,flex:"1 1 150px"}} type="date" value={t.deadline||""} onChange={e=>update(t.id,"deadline",e.target.value)}/><select style={{...is,width:150,cursor:"pointer"}} value={getCat(t)} onChange={e=>update(t.id,"category",e.target.value)}>{CCOLS.map(c=><option key={c.k} value={c.k}>{c.l}</option>)}</select><input style={{...is,width:130}} type="number" min="1" max="31" value={t.recurDay||""} onChange={e=>{const v=e.target.value?parseInt(e.target.value):null;update(t.id,"recurDay",v&&v>=1&&v<=31?v:null)}} placeholder="🔄 毎月X日"/></div>
      ...
      <div style={{display:"flex",gap:12,overflowX:"auto",paddingBottom:12,WebkitOverflowScrolling:"touch"}}>
        {(boardMode==="category"?CCOLS:TCOLS).map(col=>{const allBoard=[...tasks.filter(t=>!t.archived),...virtualTasks];const colTasks=allBoard.filter(t=>boardMode==="category"?getCat(t)===col.k:getCol(t)===col.k).sort(sortTasks);const colHover=dragOver&&dragOver.col===col.k&&!dragOver.id;const colDrop=e=>{e.preventDefault();const dId=dragId;sDragOver(null);sDragId(null);if(!dId||dId.startsWith("case:")||dId.startsWith("sem:"))return;if(boardMode==="category"){const last=colTasks.filter(t=>t.id!==dId&&!t.source);const pos=last.length;sT(p=>p.map(t=>t.id===dId?{...t,category:col.k,manualPos:pos+1}:t))}else moveTo(dId,col.k)};const colOver=e=>{if(!dragId||dragId.startsWith("case:")||dragId.startsWith("sem:"))return;e.preventDefault();e.dataTransfer.dropEffect="move";sDragOver({col:col.k,id:null})};return <div key={col.k} onDragOver={colOver} onDragLeave={e=>{if(!e.currentTarget.contains(e.relatedTarget))sDragOver(null)}} onDrop={colDrop} style={{flex:"0 0 280px",background:colHover?col.hd:col.bg,borderRadius:14,padding:12,border:colHover?"2px solid "+col.t:"1.5px solid "+col.hd,display:"flex",flexDirection:"column",maxHeight:"calc(100vh - 240px)",transition:"background .12s,border-color .12s"}}>
          ...
          <div onDragOver={colOver} onDrop={colDrop} style={{flex:1,overflowY:"auto",minHeight:60,marginRight:-4,paddingRight:4}}>
            {colTasks.length===0&&<div style={{fontSize:11,color:"#B0BEC5",textAlign:"center",padding:"16px 0"}}>タスクなし</div>}
            {colTasks.map(t=>{const dy=dt(t.deadline);const urg=dy!==null&&dy<=0;const warn=dy!==null&&dy>0&&dy<=3;const isV=!!t.source;const dOver=dragOver&&dragOver.id===t.id?dragOver.pos:null;return <div key={t.id} draggable={!isV} onDragStart={e=>{if(isV){e.preventDefault();return}e.dataTransfer.effectAllowed="move";sDragId(t.id)}} onDragEnd={()=>{sDragId(null);sDragOver(null)}} onDragOver={e=>{if(!dragId||dragId===t.id||dragId.startsWith("case:")||dragId.startsWith("sem:"))return;e.preventDefault();e.stopPropagation();e.dataTransfer.dropEffect="move";const r=e.currentTarget.getBoundingClientRect();sDragOver({id:t.id,pos:e.clientY<r.top+r.height/2?"before":"after"})}} onDragLeave={()=>{}} onDrop={e=>dropOnCard(t.id,col.k,colTasks,e)} onClick={()=>sExp(e=>({...e,[t.id]:!e[t.id]}))} style={{background:"#fff",borderRadius:10,padding:"10px 12px",marginBottom:8,cursor:isV?"pointer":"grab",border:urg?"1.5px solid #EF9A9A":warn?"1.5px solid #FFCC80":"1px solid "+col.hd,opacity:dragId===t.id?.4:1,boxShadow:"0 1px 3px rgba(0,0,0,.05)",borderTop:dOver==="before"?"3px solid #1976D2":undefined,borderBottom:dOver==="after"?"3px solid #1976D2":undefined,position:"relative"}}>
              ...
              <div style={{fontSize:13,fontWeight:600,color:"#1A2A3A",textDecoration:(t.done||(boardMode==="status"&&col.k===3))?"line-through":"none",opacity:(t.done||(boardMode==="status"&&col.k===3))?.6:1,wordBreak:"break-word"}}>{t.recurDay&&<span style={{fontSize:9,fontWeight:700,padding:"1px 5px",borderRadius:5,background:"#E1F5FE",color:"#01579B",marginRight:5}}>🔄{t.recurDay}日</span>}{t.title}</div>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:8,gap:4}}>
                <div style={{display:"flex",gap:3,flexWrap:"wrap"}}>
                  ...
                  {isV?<button onClick={e=>{e.stopPropagation();toggleVirtual(t.id)}} title="完了してログに記録" style={{padding:"3px 10px",height:22,borderRadius:6,border:"1px solid #81C784",background:"#E8F5E9",fontSize:10,cursor:"pointer",color:"#1B5E20",fontWeight:700}}>✓ 完了</button>:<button onClick={e=>{e.stopPropagation();completeOne(t)}} title={t.recurDay?"完了して次回("+t.recurDay+"日)へ":"完了＆アーカイブ"} style={{padding:"3px 10px",height:22,borderRadius:6,border:"1px solid #81C784",background:"#E8F5E9",fontSize:10,cursor:"pointer",color:"#1B5E20",fontWeight:700}}>✓ 完了</button>}
                </div>
