...
function CaseMod(){
  ...
  const fil=cases.filter(c=>{if(isDone(c.status))return false;if(fs!=="すべて"&&c.status!==fs)return false;if(q)return(c.name+c.client+c.assignee+c.memo+(c.nextAction||"")).toLowerCase().includes(q.toLowerCase());return true}).sort((a,b)=>{const ka=a.status==="検討中"?1:0,kb=b.status==="検討中"?1:0;if(ka!==kb)return ka-kb;if(a.order!=null&&b.order!=null)return a.order-b.order;if(a.order!=null)return-1;if(b.order!=null)return 1;return(a.shootDate||"9999").localeCompare(b.shootDate||"9999")});
  ...
  <div style={{maxWidth:720,margin:"0 auto",padding:"20px 16px 40px"}}>
    ...
    {vw==="cases"&&<><div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap"}}><div style={{flex:1,minWidth:180}}><input style={{...is,background:"#fff"}} placeholder="検索…" value={q} onChange={e=>sQ(e.target.value)}/></div><div style={{display:"flex",gap:4,alignItems:"center",flexWrap:"wrap"}}>{["すべて",...STS.filter(s=>!isDone(s))].map(s=>{const a=fs===s,sc=s!=="すべて"?SC[s]:null;return <button key={s} onClick={()=>sFs(s)} style={{padding:"7px 14px",borderRadius:20,border:a?"none":"1.5px solid #E0E6EB",background:a?(sc?sc.bg:"#1A2A3A"):"#fff",color:a?(sc?sc.t:"#fff"):"#78909C",fontSize:12,fontWeight:600,cursor:"pointer"}}>{s}</button>})}</div></div>{fil.length===0?<div style={{textAlign:"center",padding:60,color:"#B0BEC5"}}>{cases.length===0?"まだ案件がありません。":"該当なし"}</div>:fil.map(c=><CC key={c.id} c={c} onEdit={c=>{sE(c);sM(true)}} onDel={id=>sD(id)} onDragStart={onDragStart} onDrop={handleDrop} onDragEnd={onDragEnd} isDragging={dragId===c.id} dragOver={dragOverId===c.id&&dragId!==c.id}/>)}</>}
