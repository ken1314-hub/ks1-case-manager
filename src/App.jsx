...
function CaseMod(){
  ...
  const sortFn=(a,b)=>{const ka=a.status==="検討中"?1:0,kb=b.status==="検討中"?1:0;if(ka!==kb)return ka-kb;if(sortBy==="manual"){if(a.order!=null&&b.order!=null)return a.order-b.order;if(a.order!=null)return-1;if(b.order!=null)return 1}const gk=c=>sortBy==="shootDate"?(c.shootDate||c.publishDate||c.deadline||"9999-12-31"):(c.publishDate||c.shootDate||c.deadline||"9999-12-31");return gk(a).localeCompare(gk(b))};
...
function SemMod(){
  ...
  const fil=sems.filter(c=>{if(isDone(c.status))return false;if(fs!=="すべて"&&c.status!==fs)return false;if(q)return(c.name+c.client+c.venue+c.memo+(c.nextAction||"")).toLowerCase().includes(q.toLowerCase());return true}).sort((a,b)=>{const ka=a.status==="検討中"?1:0,kb=b.status==="検討中"?1:0;if(ka!==kb)return ka-kb;const ad=a.eventDate||"9999-12-31",bd=b.eventDate||"9999-12-31";if(ad!==bd)return ad.localeCompare(bd);return(a.name||"").localeCompare(b.name||"")});
