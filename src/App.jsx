          ...
          <div style={{flex:1,overflowY:"auto",minHeight:30,marginRight:-4,paddingRight:4}}>
            ...
            {colTasks.map(t=>{const dy=dt(t.deadline);const urg=dy!==null&&dy<=0;const warn=dy!==null&&dy>0&&dy<=3;const isV=!!t.source;return <div key={t.id} draggable={!isV} onDragStart={e=>{if(isV){e.preventDefault();return}e.dataTransfer.effectAllowed="move";sDragId(t.id)}} onDragEnd={()=>sDragId(null)} onClick={()=>sExp(e=>({...e,[t.id]:!e[t.id]}))} style={{background:"#fff",borderRadius:10,padding:"10px 12px",marginBottom:8,cursor:isV?"pointer":"grab",border:urg?"1.5px solid #EF9A9A":warn?"1.5px solid #FFCC80":"1px solid "+col.hd,opacity:dragId===t.id?.4:1,boxShadow:"0 1px 3px rgba(0,0,0,.05)"}}>
              ...
              <div style={{display:"flex",alignItems:"flex-start",gap:8}}>
                {!isV&&<button onClick={e=>{e.stopPropagation();toggle(t.id)}} title={t.done?"未完了に戻す":"完了"} style={{width:20,height:20,borderRadius:"50%",border:"2px solid "+(t.done?"#4CAF50":"#1976D2"),background:t.done?"#4CAF50":"#fff",cursor:"pointer",flexShrink:0,padding:0,display:"flex",alignItems:"center",justifyContent:"center",marginTop:1}}><span style={{color:t.done?"#fff":"#1976D2",fontSize:12,fontWeight:700,visibility:t.done?"visible":"hidden"}}>✓</span></button>}
                <div style={{fontSize:13,fontWeight:600,color:"#1A2A3A",textDecoration:(t.done||(boardMode==="status"&&col.k===3))?"line-through":"none",opacity:(t.done||(boardMode==="status"&&col.k===3))?.6:1,wordBreak:"break-word",flex:1}}>{t.title}</div>
              </div>
