...
function CC({c,onEdit,onDel}){
  const amt=pa(c.amount),ito=pa(c.itoShare);
  return <div style={{background:"#fff",borderRadius:14,padding:"18px 20px",marginBottom:10,border:"1px solid #ECEFF1",cursor:"pointer"}} onClick={()=>onEdit(c)}>
...
function CaseMod(){
  ...
  const fil=cases.filter(c=>{if(c.status==="完了")return false;if(fs!=="すべて"&&c.status!==fs)return false;if(q)return(c.name+c.client+c.assignee+c.memo+(c.nextAction||"")).toLowerCase().includes(q.toLowerCase());return true});
