...
function CC({c,onEdit,onDel,onDragStart,onDrop,onDragEnd,isDragging}){
  const amt=pa(c.amount),ito=pa(c.itoShare);
  const touchRef=useRef(null);
  const handleTouchStart=e=>{touchRef.current={id:c.id,y:e.touches[0].clientY,moved:false};};
  const handleTouchMove=e=>{if(!touchRef.current)return;if(Math.abs(e.touches[0].clientY-touchRef.current.y)>10)touchRef.current.moved=true;};
  const handleTouchEnd=e=>{if(touchRef.current&&!touchRef.current.moved){onEdit(c)}touchRef.current=null;};
  return <div draggable onDragStart={e=>{e.dataTransfer.effectAllowed="move";onDragStart&&onDragStart(c.id)}} onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();onDrop&&onDrop(c.id)}} onDragEnd={onDragEnd} style={{background:"#fff",borderRadius:14,padding:"18px 20px",marginBottom:10,border:isDragging?"2px dashed #42A5F5":"1px solid #ECEFF1",cursor:"grab",opacity:isDragging?.5:1,transition:"opacity 0.2s"}} onClick={()=>onEdit(c)}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
      <div style={{flex:1,minWidth:0}}>
        ...
        {(c.shootDate||c.publishDate)&&<div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap",marginTop:8,paddingTop:8,borderTop:"1px dashed #ECEFF1"}}>{c.shootDate&&<span style={{fontSize:11,color:"#5C6BC0",background:"#E8EAF6",padding:"2px 10px",borderRadius:12,fontWeight:600}}>{c.shootType||"取材"} {fd(c.shootDate)}{c.shootTime?" "+c.shootTime:""}</span>}{c.publishDate&&<span style={{fontSize:11,color:"#00897B",background:"#E0F2F1",padding:"2px 10px",borderRadius:12,fontWeight:600}}>配信 {fd(c.publishDate)}</span>}</div>}
...
function CaseMod(){
  ...
  const[dragId,sDragId]=useState(null);
  const fil=cases.filter(c=>{if(c.status==="完了")return false;if(fs!=="すべて"&&c.status!==fs)return false;if(q)return(c.name+c.client+c.assignee+c.memo+(c.nextAction||"")).toLowerCase().includes(q.toLowerCase());return true}).sort((a,b)=>{if(a.order!=null&&b.order!=null)return a.order-b.order;if(a.order!=null)return-1;if(b.order!=null)return 1;return(a.shootDate||"9999").localeCompare(b.shootDate||"9999")});
  const onDragStart=(id)=>sDragId(id);
  const onDrop=(targetId)=>{if(!dragId||dragId===targetId)return;const ids=fil.map(c=>c.id);const fromIdx=ids.indexOf(dragId);const toIdx=ids.indexOf(targetId);if(fromIdx<0||toIdx<0)return;ids.splice(fromIdx,1);ids.splice(toIdx,0,dragId);const orderMap=Object.fromEntries(ids.map((id,i)=>[id,i]));sC(p=>p.map(c=>orderMap[c.id]!=null?{...c,order:orderMap[c.id]}:c));sDragId(null)};
  const onDragEnd=()=>sDragId(null);
