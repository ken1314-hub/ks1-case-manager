...
function CaseForm({init,onSave,onCancel,opts}){
  const df={name:"",client:"",status:"検討中",assignee:"",deadline:"",shootType:"取材",shootDate:"",shootTime:"",publishDate:"",nextAction:"",nextActionDeadline:"",nextActionMemo:"",caseType:"取材編",amount:"",itoShare:"",itoManual:false,memo:"",actionLog:[]};
...
function SemMod(){
  ...
  const fil=sems.filter(c=>{if(isDone(c.status))return false;if(fs!=="すべて"&&c.status!==fs)return false;if(q)return(c.name+c.client+c.venue+c.memo+(c.nextAction||"")).toLowerCase().includes(q.toLowerCase());return true});
